...
Connection::Connection(EventLoop *loop, Scope<Socket> clientSocket)
    : m_Loop(loop), m_ClientSocket(std::move(clientSocket)),
      m_Disconnect(false) {
...
Connection::~Connection() { printf("Connection(fd=%d)已析构\n", GetFd()); }

int Connection::GetFd() const { return m_ClientSocket->GetFd(); }

std::string Connection::GetIp() const { return m_ClientSocket->GetIp(); }

uint16_t Connection::GetPort() const { return m_ClientSocket->GetPort(); }
...
void Connection::OnMessage() {
  ...
  while (true) {
    ...
    if (nread > 0) {
      ...
      std::string message;
      while (true) {
        if (!m_InputBuffer.PickMessage(message))
          break;
...
void Connection::SetMessageCallback(
    std::function<void(Ref<Connection>, std::string &)> fn) {
  m_MessageCallback = fn;
}

void Connection::SetSendCompleteCallback(
    std::function<void(Ref<Connection>)> fn) {
